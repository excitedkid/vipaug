...
parser.add_argument('--data', metavar='DIR',
                    help='path to dataset', default='/ws/imagenet',)
...
parser.add_argument('--nonvital', type=float, default=0.005) #set the argument depending on datasets cifar10: 0.014, cifar100: 0.012, imagenet:  0.005
...
parser.add_argument('--fractal_path', type=str, default='./fractals/images_224/') #change 'images_32' to 'images_224' in ImageNet
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    transforms.Compose([
        ...
        transforms.RandomApply([VIPAug(args.kernel, args.vital, args.nonvital, dataset_name='imagenet', fractal_images=args.fractal_path)], p=1.0),
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, targets) in enumerate(train_loader):
        ...
        images_mix = Variable(images_mix)
...
def save_checkpoint(state, is_best, filename='checkpoints/checkpoint.pth.tar', args=None):
    filename = './results/checkpoints/{}_{}_checkpoint.pth.tar'.format(args.arch, args.conf)
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, './results/checkpoints/{}_{}_best.pth.tar'.format(args.arch, args.conf))
